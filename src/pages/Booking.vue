    ...
    <section class="main">
      <div class="container">
        ...
        <div v-if="currentStep === 1" class="content">
          ...
          <div class="grid">
            ...
            >
              ...
              <!-- ✅ แสดง Multiplier แทนราคา -->
              <div class="multiplier-badge">
                <span class="multiplier-value">×{{ vehicle.multiplier }}</span>
                <span class="multiplier-label">
                  {{ vehicle.multiplier > 1 ? `+${((vehicle.multiplier - 1) * 100).toFixed(0)}%` : 
                     vehicle.multiplier < 1 ? `${((vehicle.multiplier - 1) * 100).toFixed(0)}%` : 'มาตรฐาน' }}
                </span>
              </div>
        ...
        <div v-if="currentStep === 2" class="content">
          ...
          <!-- ✅ Summary แบบใหม่ (แสดงราคาหลังคูณ multiplier) -->
              ...
              <div class="services-list">
                <div v-for="id in selectedServices" :key="id" class="service-item">
                  <span class="service-name">
                     <img :src="getServiceIcon(id)" class="service-item-icon"/> 
                     {{ getServiceName(id) }}
                     <small>(฿{{ getServicePrice(id) }} × {{ getVehicleMultiplier(selectedVehicle) }})</small>
                  </span>
...
// (ใช้โค้ด script ที่ส่งมาทั้งหมด - ไม่ต้องแก้)
...
const carTypes = [
  { id: 'sedan', name: 'รถเก๋ง', desc: 'Sedan', size: 'M', multiplier: 1.00, image: '/icons/sedan.svg' },
  { id: 'pickup', name: 'รถกระบะ', desc: 'Pickup', size: 'L', multiplier: 1.20, image: '/icons/pickup.svg' },
  { id: 'sports', name: 'รถสปอร์ต', desc: 'Sports', size: 'M', multiplier: 1.50, image: '/icons/sports.svg' },
  { id: 'van', name: 'รถตู้', desc: 'Van', size: 'XL', multiplier: 1.30, image: '/icons/van.svg' },
  { id: 'motorcycle', name: 'มอเตอร์ไซค์', desc: 'Bike', size: 'S', multiplier: 0.80, image: '/icons/motorcycle.svg' }
];
...
const selectVehicle = (id: string) => { selectedVehicle.value = id; };
...
const getVehicleMultiplier = (id: string) => carTypes.find(v => v.id === id)?.multiplier || 1.00;
  ...
  Swal.fire({
    ...
    html: `
      <div style="text-align: left; max-height: 400px; overflow-y: auto; padding: 1rem;">
        <h4 style="margin-bottom: 1rem;">📋 เงื่อนไขการจอง</h4>
        <p style="margin-bottom: 0.75rem;">• กรุณามาตรงเวลาที่จอง หากมาสายเกิน 15 นาที ร้านขอสงวนสิทธิ์ยกเลิกคิว</p>
        <p style="margin-bottom: 0.75rem;">• ยกเลิก/เลื่อนนัด ต้องแจ้งล่วงหน้าอย่างน้อย 2 ชั่วโมง</p>
        <p style="margin-bottom: 0.75rem;">• ราคาบริการคำนวณจาก: ราคาบริการ × ตัวคูณของรถ</p>
        <p>• ส่วนลดพิเศษเมื่อจอง 2 บริการขึ้นไป</p>
      </div>
    `,
...
const confirmBooking = async () => {
  try {
    ...
    const response = await axios.post('http://localhost:3000/api/booking/create', {
      ...
      vehicle_multiplier: getVehicleMultiplier(selectedVehicle.value),
      ...
      Swal.fire({
        ...
        html: `
          <div style="padding: 1rem;">
            ...
            <p style="color: #10b981; margin-bottom: 0.5rem;">⏱️ ใช้เวลาประมาณ ${totalServiceDuration.value} นาที</p>
            <p style="color: #fbbf24;">💰 ยอดชำระ: ฿${total.value.toLocaleString()}</p>
          </div>
        `,
        icon: 'success',
...
<style scoped>
/* (ใช้ CSS ที่ส่งมาทั้งหมด + CSS ใหม่ด้านล่าง) */
* {
  margin: 0;
  padding: 0;
...
/* ✅ Multiplier Badge (แทน Price Badge) */
.multiplier-badge {
  display: flex;
...
/* (CSS ที่เหลือเหมือนเดิม - copy จากที่ส่งมา) */
.car-image,
.service-image {
  width: 100%;
  height: 90px;
